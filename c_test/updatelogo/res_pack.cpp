...
int updateUbootLogo(const char* path){
	...
	const char Logofile[]="/cache/updatelogo.img";
	const char Srclogo[]="/cache/srclogo.img";
	const char Logoblock[]="/dev/block/logo";
	const char Tmpdir[]="/cache/tmplogodir";
	AmlResImgHead_t headinfo[1];
	
	sprintf(tmp,"rm -rf %s;",Srclogo);
	system(tmp);
	
	fi=fopen(Logoblock,"r");
	fo=fopen(Srclogo,"w");
	if(fi==NULL){
		LOGI("logo block %s Open Error!\n",Logoblock);
		return -1;
	}
	if(fo==NULL){
		LOGI("logo file %s Open Error!\n",Srclogo);
		return -1;
	}
	fread(headinfo,sizeof(AmlResImgHead_t),1,fi);
	fseek(fi,0,SEEK_SET);
	
	count=(long)headinfo[0].imgSz;
	LOGI("count is  %ld\n",count );
	while(count>0){
		count--;
		fscanf(fi,"%c",&c);
...
//	sprintf(tmp,"chmod 777 %s;",Srclogo);
//	strcat(tmpcmd,tmp);
	
	sprintf(tmp,"rm -rf %s;",Tmpdir);
	strcat(tmpcmd,tmp);
	sprintf(tmp,"rm -rf %s;",Logofile);
	strcat(tmpcmd,tmp);
	sprintf(tmp,"mkdir -p %s;",Tmpdir);
	strcat(tmpcmd,tmp);
//	sprintf(tmp,"chmod -R 777 %s;",Tmpdir);
//	strcat(tmpcmd,tmp);

	system(tmpcmd);
	LOGI("step1 is :  %s",tmpcmd);
    bzero(tmpcmd,sizeof(tmpcmd));
    bzero(tmp,sizeof(tmp));

	ret = res_img_unpack(Srclogo,Tmpdir,1);
	if(ret != 0){
		LOGI("res_img_unpack() is faied %d \n",__LINE__);
		return __LINE__;
	}

	LOGI("step2 is : imgpack -d %s %s;",Srclogo,Tmpdir);
//	sprintf(tmp,"imgpack -d %s %s;",Srclogo,Tmpdir);
	sprintf(tmp,"cp %s/%s %s/bootup;",path,pic_name,Tmpdir);
	system(tmp);
	LOGI("step3 is :  %s",tmp);
    bzero(tmp,sizeof(tmp));
	
	ret = res_img_pack(Tmpdir,Logofile);
	if(ret != 0){
		LOGI("res_img_pack() is faied %d \n",__LINE__);
		return __LINE__;
	}
	LOGI("setp4 is : imgpack -r %s %s;",Tmpdir,Logofile);

#if 0

	sprintf(tmp,"cat %s > %s", Logofile, Logoblock);
	system(tmp);
	LOGI("step5 is :  %s",tmp);
#else	
    fi=fopen(Logofile,"r");
	fo=fopen(Logoblock,"w");
	if(fi==NULL){
		LOGI("logo file %s Open Error!\n",Logofile);
		goto error0;
	}
	...
	fclose(fo);
#endif	
	LOGI("logo upgrade success \n");

	sprintf(tmp,"rm -rf %s %s %s", Logofile, Srclogo, Tmpdir);
	system(tmp);
...
int main(int argc, char *argv[])
{
	...
	wd = inotify_add_watch(fd, argv[1], IN_DELETE | IN_CREATE);
	if(wd < 0)
	{
		LOGI("inotify_add_watch %s failed\n", argv[1]);
		return -1;
	}
			...
			for(i=0; i<EVENT_NUM; i++)
			{
				if((event->mask >> i) & 1)
				{
					...
					{	
						...
						if((strcmp(event->name,pic_name) == 0) && (strcmp(event_str[i],"IN_CREATE") == 0))
						{
							LOGI("logo %s is  %s  and now update bootlogo",event->name,event_str[i]);
							updateUbootLogo(argv[1]);
						}
