...
#define IR_MAX_SUPPORT_NUM 20

#ifdef CONFIG_HAS_EARLYSUSPEND
#include <linux/earlysuspend.h>
static struct early_suspend early_suspend;
#endif
extern void call_irled_blink(void);
static bool key_pointer_switch = true;
static unsigned int FN_KEY_SCANCODE[IR_MAX_SUPPORT_NUM] = {0};
static unsigned int OK_KEY_SCANCODE[IR_MAX_SUPPORT_NUM] = {0};
...
static __u16 key_map[IR_MAX_SUPPORT_NUM][512];
...
static __u16 mouse_map[IR_MAX_SUPPORT_NUM][6];
...
{
    printk(KERN_DEBUG "remote_send_key \n");

    printk(KERN_DEBUG "mouse_key=%d,ok_key=%d\n", FN_KEY_SCANCODE[gp_remote->map_num], OK_KEY_SCANCODE[gp_remote->map_num]);

	if(scancode == FN_KEY_SCANCODE[gp_remote->map_num] && type == 1)
    {
    ...
    if(scancode == OK_KEY_SCANCODE[gp_remote->map_num] && key_pointer_switch == false)
    {
...
#ifdef CONFIG_IR_REMOTE_DBG
    printk("remote_config_ioctl cmd=%x\n", cmd);
#endif

	switch (cmd) {
        case REMOTE_IOC_GET_MAX_SUPPORT_IR_NUM	   :
             val = IR_MAX_SUPPORT_NUM;
			 ret = copy_to_user(argp, &val, sizeof(long));
             break;
        case REMOTE_IOC_INFCODE_CONFIG:
            printk("config map_num=%d\n", val);
            if(val >= IR_MAX_SUPPORT_NUM)
                return -1;
			remote->map_num = val;
			break;
		case REMOTE_IOC_RESET_KEY_MAPPING:
            ...
            FN_KEY_SCANCODE[remote->map_num] = 0xFFFFFFFF;
            OK_KEY_SCANCODE[remote->map_num] = 0xFFFFFFFF;
			break;
		...
		case REMOTE_IOC_SET_MOUSE_MAPPING:
			...
			mouse_map[remote->map_num][val >> 16] = val & 0xffff;
			break;
		...
		case REMOTE_IOC_SET_FN_KEY_SCANCODE:
            printk("FN_KEY_SCANCODE=%d\n", val); 
			FN_KEY_SCANCODE[remote->map_num] = val;
			break;
		case REMOTE_IOC_SET_OK_KEY_SCANCODE:
            printk("OK_KEY_SCANCODE=%d\n", val);
			OK_KEY_SCANCODE[remote->map_num] = val;
			break;
	}
	//output result
	switch (cmd) {
