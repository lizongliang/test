                                       ...
                                       DeviceInfo* info)
{
    ...
    // Only check for our device
    len = s_get_alsa_card_name(cardName, sizeof(cardName), pcmCard);
    if (len <= 0 || strcmp(DIA_REMOTE_AUDIO_DEVICE_ID, cardName) != 0) {
        return false;
    }
    ALOGD("AudioHotplugThread: found %s virtual sound card", cardName);
    ...
    ALOGD("AudioHotplugThread: %s parameters, card=%d, pcm=%d, sample-bits=%d..%d, channels=%d..%d, sample-rate=%d..%d",
          cardName, info->pcmCard, info->pcmDevice, info->minSampleBits, info->maxSampleBits ,
          info->minChannelCount, info->maxChannelCount, info->minSampleRate, info->maxSampleRate);
