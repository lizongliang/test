...
#define DIA_REMOTE_AUDIO_DEVICE_NAME_LONG	"DIALOG Remote %i audio" // max=80
...
{
	uint i;
	struct snd_dialog *dia_snd = substream != NULL ? snd_pcm_substream_chip(substream) : s_dia_snd;
...
static void audio_dec(const uint8_t *raw_input, int type, size_t num_bytes)
{
	...
	if (substream != NULL) {
		...
		if (writable > 0) {
			...
			packet->start = packet->pos == dia_snd->stream_enable_pos;
			packet->reset = packet->pos == dia_snd->stream_reset_pos;
		...
		if (s_dia_snd != NULL) {
			...
			if (s_dia_snd->stream_reset_pos == s_dia_snd->num_bytes) {
			...
			// the packet in the FIFO. It will be decoded when snd_dialog_pcm_close is called.
			if (atomic_fifo_available_to_read(&s_dia_snd->fifo_controller) > 0
			    && atomic_fifo_available_to_write(&s_dia_snd->fifo_controller) > 0) {
				uint fifo_index = atomic_fifo_get_write_index(&s_dia_snd->fifo_controller);
				struct fifo_packet *packet = &s_dia_snd->fifo_packet_buffer[fifo_index];
				packet->type = type;
				packet->num_bytes = (uint8_t)num_bytes;
				packet->pos = s_dia_snd->num_bytes;
				memcpy(packet->raw_data, raw_input, num_bytes);
				atomic_fifo_advance_write(&s_dia_snd->fifo_controller, 1);
			} else {
				snd_dialog_decode_adpcm_packet(NULL, raw_input, num_bytes);
			}
	...
	if (dropped_packet)
		snd_dialog_log("WARNING, raw audio packet dropped, FIFO full\n");
}
 ...
 */
static uint snd_dialog_decode_from_fifo(struct snd_pcm_substream *substream, uint *max_frames)
{
	uint i, frames;
	struct snd_dialog *dia_snd = snd_pcm_substream_chip(substream);
	...
	for (i = 0; i < readable; i++) {
		...
		// Check buffer size
		if (frames + packet->num_bytes * 8 / DECODER_BITS_PER_SAMPLE > dia_snd->frames_per_buffer - 4) {
			*max_frames = frames;
			break;
		}
		...
		if (frames >= *max_frames /*&& (int) (dia_snd->num_bytes - dia_snd->decoder_pos) <= dia_snd->decoder_buffer_size*/)
			break;
	}
...
static void snd_dialog_timer_callback(unsigned long data)
{
	...
	frames_to_advance = frames_to_silence = snd_dialog_calc_frame_advance(dia_snd);
	...
	case TIMER_STATE_DURING_DECODE:
		...
		if (atomic_fifo_available_to_read(&dia_snd->fifo_controller) > 0) {
			fifo_index = atomic_fifo_get_read_index(&dia_snd->fifo_controller);
		...
		if (s_substream_for_btle != NULL
			&& dia_snd->buffer_start_pos == dia_snd->stream_enable_pos
			&& (int) (dia_snd->buffer_start_pos - dia_snd->stream_disable_pos) >= 0
			&& dia_snd->num_bytes - dia_snd->buffer_start_pos < dia_snd->decoder_buffer_size)
		{
			pr_info("BUFFER: %d of %d", dia_snd->num_bytes - dia_snd->buffer_start_pos, dia_snd->decoder_buffer_size);
			need_silence = true;
			break;
		} else {
			pr_info("TIMER: %d %d", dia_snd->num_bytes, atomic_fifo_available_to_read(&dia_snd->fifo_controller));
		}
		// Check if we have already decoded the needed packets
		if (dia_snd->decoder_frames_adjust < 0 && -dia_snd->decoder_frames_adjust >= frames_to_advance) {
		...
		if (frames_to_advance > dia_snd->frames_per_buffer - 4) {
			dia_snd->decoder_frames_adjust = frames_to_advance - (dia_snd->frames_per_buffer - 4);
		...
		frames_decoded = snd_dialog_decode_from_fifo(substream, &frames_to_advance);
		dia_snd->decoder_frames_adjust += frames_to_advance - frames_decoded;
		if (dia_snd->decoder_frames_adjust > DECODER_MAX_ADJUST_FRAMES)
			dia_snd->decoder_frames_adjust = DECODER_MAX_ADJUST_FRAMES;
		snd_dialog_log("DECODE: %u of %u, %u %d\n", frames_decoded, frames_to_advance, dia_snd->decoder_pos, dia_snd->decoder_frames_adjust);
		if (frames_decoded > 0) {
			if ((int) (dia_snd->decoder_pos - dia_snd->stream_disable_pos) > 0 && frames_to_advance > frames_decoded)
				pr_err("Missing %u frames, decoded %u\n", frames_to_advance - frames_decoded, frames_decoded);
			break;
		} else {
			if ((int) (dia_snd->decoder_pos - dia_snd->stream_disable_pos) > 0)
				pr_err("Buffer empty, generating silence\n");
			dia_snd->decoder_frames_adjust = 0;
			// no packets read
			//snd_dialog_log("No packets read, generating silence");
			need_silence = true;
		}
...
static void snd_dialog_timer_stop(struct snd_pcm_substream *substream, bool sync)
{
	int ret;
	struct snd_dialog *dia_snd = snd_pcm_substream_chip(substream);
...
static int snd_dialog_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		if (dia_snd->timer_state != TIMER_STATE_UNINIT && try_to_del_timer_sync(&dia_snd->decoding_timer) < 0)
			return -EBUSY;
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		...
		snd_dialog_log("%s stopping audio, peak = %d, # packets = %d\n",
...
static int snd_dialog_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	dia_snd->decoder_buffer_size = runtime->rate * DECODER_BUFFER_LATENCY_MS * DECODER_BITS_PER_SAMPLE / 1000 / 8;
...
static int snd_dialog_pcm_close(struct snd_pcm_substream *substream)
{
	...
	if (dia_snd->fifo_packet_buffer) {
		...
		uint i, readable = atomic_fifo_available_to_read(&dia_snd->fifo_controller);
		if (readable)
			snd_dialog_log("%s: Decoding %d remaining FIFO packets\n", __func__, readable);
		for (i = 0; i < readable; i++) {
			uint fifo_index = atomic_fifo_get_read_index(&dia_snd->fifo_controller);
			struct fifo_packet *packet = &dia_snd->fifo_packet_buffer[fifo_index];
			snd_dialog_decode_adpcm_packet(NULL, packet->raw_data, packet->num_bytes);
			dia_snd->decoder_pos = packet->pos + packet->num_bytes;
			atomic_fifo_advance_read(&dia_snd->fifo_controller, 1);
		}
...
static int dia_snd_initialize(struct hid_device *hdev)
{
	...
	err = snd_card_create(index[dev], DIA_REMOTE_AUDIO_DEVICE_ID, THIS_MODULE,
	...
	strncpy(card->driver, DIA_REMOTE_AUDIO_DRIVER_NAME, sizeof(card->driver));
	strncpy(card->shortname, DIA_REMOTE_AUDIO_DEVICE_NAME_SHORT, sizeof(card->shortname));
	snprintf(card->longname, sizeof(card->longname), DIA_REMOTE_AUDIO_DEVICE_NAME_LONG, dev + 1);
	...
	if (report->id == ADPCM_AUDIO_REPORT_ID_0 && data[2] == 0) {
		...
		if (substream != NULL || s_dia_snd != NULL) {
			...
			if (data[1]) {
				...
				if (substream == NULL) {
					snd_dialog_log("Stream enabled with no substream. Reset decoder.\n");
					...
					// Flush the FIFO
					spin_lock(&s_substream_lock);
					atomic_fifo_init(&dia_snd->fifo_controller, MAX_PACKETS_PER_BUFFER);
					spin_unlock(&s_substream_lock);
				}
			} else {
				dia_snd->stream_disable_pos = dia_snd->num_bytes;
			}
		} 
	}
...
static int dia_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	if (!supported) {
		ret = -ENODEV;
		pr_err("Error! Device name not compatible with DIA HID: %s", hdev->name);
		goto err_match;
	}
...
static const struct hid_device_id dia_devices[] = {
	{HID_BLUETOOTH_DEVICE(USB_VENDOR_ID_DIA_0,
                              USB_DEVICE_ID_DIA_REMOTE_0)},
